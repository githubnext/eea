...
In existing software development, it is often the case that "Code is King" - the code is the primary artifact, and any specifications or documentation are secondary, or even non-existent. While the role of specifications varies across different software development paradigms and settings, the general trend in the 2010s has been to code being the primary artifact, with specifications at best secondary.

The advent of Generative AI has seen a rise in exploring ways to use the power of natural language within the software development process, opening possibilities of a new paradigm in software development where specifications or requirements documents are primary - or at least first-class - citizens in the development process. One early example of this was [the SpecLang project](https://githubnext.com/projects/speclang/), developed by GitHub Next in 2023. SpecLang was envisaged as a specification compiler that allows users to describe the behavior of code in natural language, aiming to provide a way for developers to write
specifications that are easy to understand and maintain, while also being machine-readable. While SpecLang was not publicly released it forms part of the background to the work described in this report.
...
There is, however, another more human problem with task-oriented natural language programming: the user must "find" the words, vocabulary and concepts by which to describe the change they intend. This problem has been known for many years in information retrieval, referred to as [the "vocabulary problem"](https://dl.acm.org/doi/10.1145/32206.32212). The vocabulary problem arises because the user must find the right words to describe their intent, and this can be difficult, especially when the user is not familiar with the codebase or the domain. Further the words chosen by the user may not be the same as the words used in the codebase. Another recurring problem is the "reference problem" - words must be found to refer to exact locations and entities in code sufficiently unambiguously.
...
![EEA Demonstrator](https://githubnext.com/projects/eea/images/video-message-language-change.mp4)
